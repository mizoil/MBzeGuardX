...
import com.follow.mbzeguardx.MainActivity
import com.follow.mbzeguardx.R
import com.follow.mbzeguardx.extensions.getActionPendingIntent
...
fun Service.createMBzeGuardXNotificationBuilder(): Deferred<NotificationCompat.Builder> =
    CoroutineScope(Dispatchers.Main).async {
        val stopText = GlobalState.getText("stop")
        val intent = Intent(this@createMBzeGuardXNotificationBuilder, MainActivity::class.java)

        val pendingIntent = if (Build.VERSION.SDK_INT >= 31) {
            PendingIntent.getActivity(
                this@createMBzeGuardXNotificationBuilder,
                0,
                intent,
            ...
            PendingIntent.getActivity(
                this@createMBzeGuardXNotificationBuilder, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT
            )
        }

        with(
            NotificationCompat.Builder(
                this@createMBzeGuardXNotificationBuilder, GlobalState.NOTIFICATION_CHANNEL
            )
        ) {
            setSmallIcon(R.drawable.ic)
            setContentTitle("MBzeGuardX")
