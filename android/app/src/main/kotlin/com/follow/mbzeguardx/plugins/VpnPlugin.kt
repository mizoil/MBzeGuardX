...
import com.follow.mbzeguardx.RunState
import com.follow.mbzeguardx.core.Core
import com.follow.mbzeguardx.extensions.awaitResult
import com.follow.mbzeguardx.extensions.resolveDns
import com.follow.mbzeguardx.models.StartForegroundParams
import com.follow.mbzeguardx.models.VpnOptions
import com.follow.mbzeguardx.services.BaseServiceInterface
import com.follow.mbzeguardx.services.MBzeGuardXService
import com.follow.mbzeguardx.services.MBzeGuardXVpnService
...
data object VpnPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    private val connectivity by lazy {
        MBzeGuardXApplication.getAppContext().getSystemService<ConnectivityManager>()
    }
    ...
    private val connection = object : ServiceConnection {
        override fun onServiceConnected(className: ComponentName, service: IBinder) {
            ...
            flClashXService = when (service) {
                is MBzeGuardXVpnService.LocalBinder -> service.getService()
                is MBzeGuardXService.LocalBinder -> service.getService()
    ...
    private fun protect(fd: Int): Boolean {
        return (flClashXService as? MBzeGuardXVpnService)?.protect(fd) == true
    }
    ...
    private fun resolverProcess(
        ...
        if (!uidPageNameMap.containsKey(nextUid)) {
            uidPageNameMap[nextUid] =
                MBzeGuardXApplication.getAppContext().packageManager?.getPackagesForUid(nextUid)
                    ?.first() ?: ""
        }
    ...
    private fun bindService() {
        if (isBind) {
            MBzeGuardXApplication.getAppContext().unbindService(connection)
        }
        val intent = when (options?.enable == true) {
            true -> Intent(MBzeGuardXApplication.getAppContext(), MBzeGuardXVpnService::class.java)
            false -> Intent(MBzeGuardXApplication.getAppContext(), MBzeGuardXService::class.java)
        }
        MBzeGuardXApplication.getAppContext().bindService(intent, connection, Context.BIND_AUTO_CREATE)
    }
}